...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\response{
			We thank the referee for this comment. We have extended our investigation on formation and stabilize the quantum chimeralike order for the system size beyond N=8. The regional magnetization ($M^z_A$) plot in figure(9) exhibits melting in DTC phase accompanied by beats at almost all possible spin interactions (defined by $\beta$) and coupling strengths in $M^z_A$ curves in region A. To understand this melting in DTC observation, we have considered sizes N =4,6,8,10 and numerically calculated $M^z_A$ at and obtained corresponding FFTs for an extended time period upto $8 \times 10^4$T at DTC/DL point.  DTC/DL point is the first root of Bessel function of drive parameters $\frac{4h}{\omega}$. \\		
