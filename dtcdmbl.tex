...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{figure}[h!]
				...
				\end{center}
				\caption{The left panel shows the local magnetization for different $\beta$ at the first root of $\mathcal{J}_0(4h/\omega)$ for $N=8$ up to 80T. In the top panels; right panel shows the local magnetization at the third root of $\mathcal{J}_0(4h/\omega)$ for the same parameters. bottom panels denotes points other thatn root of $\mathcal{J}_0(4h/\omega)$. The drive frequency is fixed at $\omega=20$ and the drive amplitude is set at the CDT/DL point. The temporal variation of $\expval{\hat{S}_z}$ is the same in both panels.}
				\label{Fig:rootissue}
			\end{figure}
		}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\response{We appreciate the referee for bringing our oversight to our attention. The labelling of y-coordinate was incorrect in figure 5. THe figure displays the FFT analysis of the local magnetization $\expval{\hat{S_z^i}}$ at a specific site $i=1$ in region A. We have made the necessary corrections and updates to figure 5 in the revised manuscript.}\\
	...
	\begin{enumerate}
		\item The referee says, \comment{``My main concern is regarding the context in which the word “chimera” has been used
			...
			\response{The referee's criticism and recommendation are appreciated. We agree with the referee that the quantum spin-1/2 chimera state is different from the classical chimera in identical oscillator systems. There are two non-homogeneous Hamiltonians in the proposed quantum model, which goes against the classical chimera phenomenon criterion of identical oscillators experiencing similar environments. This is more like the nonlocally coupled system in classical chimeralike systems~\cite{Sharma2021}. However, quantum mechanics uses linear Schrödinger dynamics, while classical dynamics can be non-linear. Thus, quantum mechanics emergent phenomena are not comparable to classical events. This raises issues about labeling a novel phenomenon a "chimera" in our suggested quantum system. We appreciate the referee's recommendation to change the phrase "chimera state" to "chimeralike state." To comply with the referee, we changed any references to 'chimera state' to 'chimeralike state' or 'chimeralike order' in the revised manuscript and title.}\\
