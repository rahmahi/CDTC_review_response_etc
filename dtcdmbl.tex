...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			A physical operator associated with the wavefunction is considered to investigate primary properties of DTC. The local magnetization $\expval{\hat{S_z}}$ is a suitable choice for this purpose. The time evolution of the local magnetization is analyzed to detect the DTC phase. The Fourier Transform of the local magnetization data is then calculated to identify the dominant subharmonic frequency. We have discussed how the time evolution of local magnetization and its FFT analysis contributes to detect DTC phase in the revised manuscript at page 3, para 2 and page 9, para 2.
			}\\
			...
			We have also analyzed the FFTs \sout{(amplitude-squares only)} of the long-time $M^z_A$ data from the extended simulations. The dominant peak is at $\Omega=\omega/2$. However, there are smaller peaks in the neighborhood, corresponding to the beats seen in the time series data.
