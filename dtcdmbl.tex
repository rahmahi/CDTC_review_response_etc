...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\response{We appreciate the referee's interest in this issue. We have conducted further investigations to explore the influence of spin rotation errors, specifically $\epsilon_B$, on the stability of the DTC-DMBL chimera-like order. In the revised manuscript, we have included Figure 7 on page 13, which presents the numerical calculation of the local magnetization $\expval{\hat{S}^z}$ for different $\beta$ values and spin coupling strengths. We have observed that as $\epsilon_B$ increases, the stability of the DMBL phase in region B decreases, which directly affects the stability of the DTC phase in region A. Additionally, we have examined the regional magnetization $M^z_A$ and $M^z_B$ at the CDT/DL point for various values of $\epsilon_A$ and $\epsilon_B$, as shown in Figure 8 on page 15. We found that the stability of the DTC-DMBL chimera-like order is highest when $\epsilon_A$ approaches 0 and $\epsilon_B$ approaches 1.0. When $\epsilon_B$ is less than 0.9 and $\epsilon_A$ is constant at 0.03, both regions gradually transition into a melting DTC phase. We have provided a comprehensive discussion on these findings in Section 3 on page 11 and Section 4.1 on page 14.}
		...
		\begin{enumerate}
			...
			\response{We appreciate the referee for bringing this issue to our attention. In order to achieve DMBL in the $T_2$ cycle, the ratio of the drive parameter ($h,\omega$) can be \textit{any} one of the roots of the Bessel function $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$.  Our numerical simulations chose the first root by default. Nonetheless, we had to choose a higher root in some simulations for technical reasons. The fact that any root will do can be seen in a comparison below in figure XX. For consistency, we have updated figures YY and ZZ in the manuscript for simulations at the first root only.
		}
		...
		\begin{enumerate}
			...
			\response{
			We thank the referee for the comment. We had stated in the earlier sections  that all-to-all interactions are the most suitable choice  for a stable chimera-like order as determined from relatively short-time simulations (around $80T$). We extended the simulations to longer times (around $2000T$) and updated the figure in the revised manuscript. The regional magnetization for all-to-all interaction and strong coupling  does not decrease appreciably even at these very large times. However, for srong couplings, appeaciable beats are observed at these longer times. Running the other cases in the figure for loger times does not change the results significantly. We have included this discussion in the revised manuscript at page 17. 
			
			We have also analyzed the FFTs (amplitude-squares only) of the long-time $M^z_A$ data from the extended simulations. The dominant peak is at $\Omega=\omega/2$, as expected. However,there are smaller peaks in the neighbourhood, corresponding to the beats seen in the time series data.
			For all-to-all interactions and weak coupling, the secondary peaks are un-noticeable. This manifests a stable chimera even at long times. However, for stronger coupling, the secondary peaks are larger. This indicates that the chimera-like order is less stable under strong coupling. We have included this discussion in the revised manuscript at page 17. 
			}
			\begin{figure}[h!]
				%\begin{center}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\response{We thank the referee for the comment. We have modified the sentence in the revised manuscript to ``The quasi-energies are plotted for different values of $h,\omega$, the amplitude, and frequency of the periodic drive, respectively''\todo[inline]{Change caption to 'The quasi-energies are plotted against $4h/\omega$, where the drive frequency $\omega=20$ is fixed, and drive amplitude $h$ is vatried.'} in order to maintain clarity in content in manuscript.}\\
		...
		\begin{enumerate}
			...
			\response{We thank the referee for the comment. As explained earlier, we had defaulted to the first root for all numerical simulations, choosing higher roots for a few selected cases. We have corrected them all to the first root in the revised manuscript and updated the caption of figure 4 accordingly.}
		\end{enumerate}
		...
		\begin{enumerate}
			...
			\response{We thank the referee for pointing out the mistake. We had labeled the y-coordinate in figure 5  incorrectly. Figure 5 presents the FFT analysis on the local magnetization at a selected site $i=1$ in region A. We have corrected and updated figure 5 in the revised manuscript with FFT plots.}\\
			\item The referee says, \comment{``Specify a CDT/DL point, i.e. to which root of the Bessel function it corresponds"}\\
			
			\response{We thank the referee for the comment. We have considered the first root of $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$ as the CDT/DL point and we have this selection of CDT/DL point consistently thorughout in the revised manuscript.}\\
		\end{enumerate}
		...
		\response{We thank the referee for the comment. We have considered the first root of $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$ as the CDT/DL point and we have this selection of CDT/DL point consistently thorughout the revised manuscript.}\\
