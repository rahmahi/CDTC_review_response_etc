...
%DIF DEL chimera_110124.tex   Fri May 31 14:50:15 2024
%DIF ADD chimera.tex          Fri May 31 14:58:25 2024
...
\begin{document}	
\title{Time \DIFdelbegin \DIFdel{Crystal Embodies Chimera }\DIFdelend \DIFaddbegin \DIFadd{crystal embodies chimeralike state }\DIFaddend in \DIFdelbegin \DIFdel{Periodically Driven Quantum Spin System}\DIFdelend \DIFaddbegin \DIFadd{periodically driven quantum spin system}\DIFaddend }
\submitto{\NJP}
...
Chimeras have also been observed in models of electrical power grids, where a synchronous state can be stabilized by tuning the parameters of the generator~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Motter2013}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Deng_2024, Motter2013}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 
...
Interest in the formation of chimeras in magnetic systems has recently increased. Curie-Weiss-type models, such as the Ising model~\cite{singh_chimera_2011}, are used to represent systems of interacting quantum spins where order and disorder coexist. Sakurai \textit{et al.}~\cite{sakurai_phys_nodate} reported the formation of a stable \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend in a one-dimensional spin-$1/2$ chain, which was achieved by surrounding a \textit{Discrete Time Crystal} (DTC) phase with another dynamical state, which is many-body localized (MBL). DTC is a novel state of matter that arises from the breaking of the discrete-time translational symmetry~\cite{else_floquet_2016}. The \DIFdelbegin \DIFdel{concept }\DIFdelend \DIFaddbegin \DIFadd{time crystal (TC) }\DIFaddend was first proposed by Frank Wilczek in 2012~\cite{wilczek_quantum_2012}, although it was later shown that such states cannot exist as an equilibrium ground state~\cite{Bruno_comment_1, Bruno2013, watanabe_absence_2015}. However, the possibility remains that \DIFdelbegin \DIFdel{a time crystal }\DIFdelend \DIFaddbegin \DIFadd{TC }\DIFaddend can be realized in a non-equilibrium system, such as Floquet systems. This has been experimentally demonstrated in several many-body systems~\cite{huang2018,taheri_all-optical_2022, Soham2018, zhang_observation_2017, yao_time_2018,frey_realization_2022, rovny_observation_2018, sacha_time_nodate,golletz_basis_2022}. \DIFaddbegin \DIFadd{Additionally, TC can occur in prethermal regimes at quasi-infinite temperatures ~\mbox{%DIFAUXCMD
\cite{Stasiuk2023} }\hspace{0pt}%DIFAUXCMD
and dissipative systems caused by controlled cavity-dissipation, interactions, and external forcing in experimental settings~\mbox{%DIFAUXCMD
\cite{Hans2021}}\hspace{0pt}%DIFAUXCMD
. Recently, a time crystal comb in open systems has been proposed for a dissipative strong interacting Rydberg gas~\mbox{%DIFAUXCMD
\cite{jiao2024}}\hspace{0pt}%DIFAUXCMD
. In addition, a continuous time crystal (CTC) where continuous time translational symmetry is broken spontaneously,  has been investigated by introducing an asymmetric subspace in a driven dissipative spin model~\mbox{%DIFAUXCMD
\cite{solanki2024}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{Periodic dynamics are typical in quantum mechanical dynamical systems. Well-known examples include Rabi oscillations~\mbox{%DIFAUXCMD
\cite{Sakurai_Napolitano_2020}}\hspace{0pt}%DIFAUXCMD
, Zitterbewegung~\mbox{%DIFAUXCMD
\cite{LeBlanc_2013}}\hspace{0pt}%DIFAUXCMD
, }\textit{\DIFadd{etc}}\DIFadd{. When a quantum system described by a state $\ket{\psi}$ undergoes evolution monitored by the propagator $e^{i H t / \hbar}$~\mbox{%DIFAUXCMD
\cite{Biao2018,russomanno_floquet_2017}}\hspace{0pt}%DIFAUXCMD
, it can display distinct oscillations. Therefore, it is crucial to establish criteria that distinguish between well-established periodic events in quantum mechanics and the emergent complex time crystal. The standard definition of a $\mathbb{Z}_2$ symmetry-breaking time crystal is that there exists an observable $\hat{O}$ (corresponding to the order parameter) associated with $\ket{\psi}$, at thermodynamic limit and with the system undergoing oscillation far from equilibrium, the quantity
...
\item 
\DIFadd{Time translation symmetry breaking (TTSB): $f(t) \neq f(t+\tau)$ while $\hat{H}(t + \tau) =  \hat{H}(t)$, 
}

\item
\DIFadd{Rigidity: $f(t)$ exhibits an oscillation of fixed period $T (T=2\tau)$ without requiring any fine-tuning of the  Hamiltonian parameters,
}

\item
 \DIFadd{Persistence: The nontrivial periodicity with a fixed time period $ T $ endures for a sufficient duration to maintain periodicity at infinity as the thermodynamic limit, $\mathrm{N} \rightarrow \infty$ is approached. 
}\end{enumerate}
  \DIFadd{Thus, the dynamics of a TC should produce a prominent peak in the spectral decomposition of $f(t)$, given by $f_\Omega$, at the TTSB frequency $\Omega=\omega_B \equiv 2\pi/\tau_B$. This manifests in the time dynamics through subharmonic modes, oscillations at integer fractions of the fundamental frequency~\mbox{%DIFAUXCMD
\cite{Biao2018, russomanno_floquet_2017}}\hspace{0pt}%DIFAUXCMD
. However, the subharmonic modes are usually unstable, melting the time crystal to the trivial infinite temperature state as predicted by the Floquet Eigenstate Thermalization Hypothesis~\mbox{%DIFAUXCMD
\cite{Mori_2018, Kim_2014, Mizuta_2020, Mori_2023_1}}\hspace{0pt}%DIFAUXCMD
. To stabilize the subharmonic modes, a widely practiced method is to introduce  disorder in the model, thus manifesting MBL in the system~\mbox{%DIFAUXCMD
\cite{zhang_observation_2017, choi_observation_2017, Khemani2016,else_discrete_2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend The MBL phase \DIFdelbegin \DIFdel{preserves }\DIFdelend \DIFaddbegin \DIFadd{tries to preserve }\DIFaddend the initial state\DIFaddbegin \DIFadd{, ideally }\DIFaddend for an indefinite period of time\DIFdelbegin \DIFdel{by introducing disorder into the system}\DIFdelend . This prevents the DTC from \DIFdelbegin \DIFdel{thermalizing, which would otherwise cause the system to reach thermal equilibrium}\DIFdelend \DIFaddbegin \DIFadd{melting and thermalizing}\DIFaddend ~\cite{zhang_observation_2017,alet_many-body_2018,else_floquet_2016,smith_many-body_2016,nguyen_signature_2021}, thus allowing the coexistence of two different phases\DIFaddbegin \DIFadd{, much like a chimera}\DIFaddend . It is now of interest to explore whether \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend chimera can exist in more general cases, such as disorder-free systems where the interactions would not normally lead to localization, but rather to thermalization\footnote{For instance, the Heisenberg interaction has off-diagonal elements, which could potentially disrupt the chimera state by allowing magnetization to travel.}.

In this paper, we explore a novel chimera consisting of a \DIFdelbegin \DIFdel{discrete-time crystal (DTC ) state }\DIFdelend \DIFaddbegin \DIFadd{DTC }\DIFaddend in a disorder-free spin-$1/2$ \DIFaddbegin \DIFadd{bipartite }\DIFaddend chain with Heisenberg exchange interactions. Instead of conventional MBL, we consider \textit{\DIFdelbegin \DIFdel{Dynamical Many-body Localization}\DIFdelend \DIFaddbegin \DIFadd{dynamical many-body localization}\DIFaddend } (DMBL)~\cite{Keser2016, haldar_dynamical_2017,haldar_dynamical_2021,bhattacharyya_transverse_2012,aditya2023dynamical,dutta2014,das_exotic_2010} through \textit{\DIFdelbegin \DIFdel{Coherent Destruction }\DIFdelend \DIFaddbegin \DIFadd{coherent destruction }\DIFaddend of \DIFdelbegin \DIFdel{Tunneling}\DIFdelend \DIFaddbegin \DIFadd{tunneling}\DIFaddend } (CDT/DL)~\cite{Grossmann1991,Kayanuma2008} by applying an external time-periodic drive that prevents thermalization \DIFdelbegin \DIFdel{of the DTC, forming the }\DIFdelend \DIFaddbegin \DIFadd{in the system. The implementation of a regional spin-flip operation, followed by a global DMBL in a periodic manner, results in the manifestation of TTSB. This leads to the emergence of a DTC phase in one region and DMBL in another region of the spin chain concurrently, a }\DIFaddend chimera state.  The external periodic transverse drive breaks $\mathbb{Z}_2$ symmetry in \DIFaddbegin \DIFadd{the spins in }\DIFaddend a manner similar to the original proposal~\cite{sakurai_phys_nodate}. We demonstrate that the thermalizing effects of the Heisenberg interaction are suppressed in the chimera state. We also show that the DTC state of the chimera is resilient to additional external static driving fields. 
\DIFdelbegin \DIFdel{Our work will be beneficial for quantum engineering and the design of quantum memory based on DTC~\mbox{%DIFAUXCMD
\cite{zhang_observation_2017}}\hspace{0pt}%DIFAUXCMD
. Furthermore, our model may provide a new insight into designing quantum neural networks, such as quantum reservoir computing systems ~\mbox{%DIFAUXCMD
\cite{Fujii_2017, Martinez_2021,Mujal_2021, Akitada2022}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin 

\DIFadd{However, this chimeralike state is distinct from the classical chimera that occurs in systems of identical oscillators. The former is built with non-homogeneous Hamiltonians, contradicting the basic criterion for generic classical chimeras built with identical oscillators under similar environments. Rather, our proposed system is analogous to classical chimeralike nonlocally coupled oscillators~\mbox{%DIFAUXCMD
\cite{Jyoti2021,nkomo_chimera_2016}}\hspace{0pt}%DIFAUXCMD
. In addition, quantum mechanics follows the linear Schr\"{o}dinger equation, while classical dynamics in chimeras have to be non-linear. Therefore, one should not expect emergent behavior in purely quantum systems that are identical to those in classical nonlinear systems. The origin of our proposed model is solely quantum mechanical and thus can be labeled as `chimeralike' following the previous works~\mbox{%DIFAUXCMD
\cite{sakurai_phys_nodate, Jyoti2021}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 

We present our work as follows: In \DIFdelbegin \DIFdel{Section}\DIFdelend \DIFaddbegin \DIFadd{section}\DIFaddend ~\ref{sec:mdl_n_dynam}, we describe the proposed spin model. In section~\ref{sec:level2}, we describe the emergence of DMBL. In \DIFdelbegin \DIFdel{Section }\DIFdelend \DIFaddbegin \DIFadd{section }\DIFaddend \ref{sec:level3}, we numerically investigate the coexistence of time crystal and DMBL phases, and support our results analytically. In \DIFdelbegin \DIFdel{Section }\DIFdelend \DIFaddbegin \DIFadd{section }\DIFaddend \ref{sec:level4}, we look at long-time stability via regional magnetization and entanglement entropy for the system, and explore the robustness of this \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend against external static fields. Finally, we discuss our results and conclude.	
...
Before investigating the \DIFdelbegin \DIFdel{chimera order }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend in our model, we explain how the Hamiltonian $\hat{H}_2$ in \DIFdelbegin \DIFdel{Eq.}\DIFdelend \DIFaddbegin \DIFadd{equation}\DIFaddend ~\eqref{eq:sysham2} exhibits CDT/DL by analytically solving for the dynamics during all $T_2-$cycles. During these times,  the entire spin system is driven by a sinusoidal transverse periodic drive $\hat{H}_D$.
...
\begin{align}
    ...
    \label{eq:movham}
\end{align}
where, in the last step, we have employed the Baker-Campbell-Hausdorff formula\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Magnus1954}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Next, we decompose the moving frame Hamiltonian into its Fourier modes and introduce the Rotating Wave Approximation (RWA), which, in the high-frequency limit, approximates the Fourier modes with their coarse-grained values over long times. This allows us to average out all but the zeroth Fourier mode (see~\ref{sec:AppendixA} for details). This yields
\begin{multline}
...
\label{eq:movham1}
\end{multline}
Now, if the drive parameters $h$ and $\omega$  are engineered in such a way that the ratio ${4h}/{\omega}$ lies at \DIFaddbegin \DIFadd{a CDT/DL point, which is given by }\DIFaddend one of the \DIFdelbegin \DIFdel{higher roots of $\mathcal{J}_0$}\DIFdelend \DIFaddbegin \DIFadd{roots of the zeroth order Bessel function $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$}\DIFaddend ~\footnote{This can be achieved when $\omega \gg J_0$ by ensuring that $h\gg J_0$.}, it is possible to nullify the dynamics of $\hat{H}_2$ during all $T_2-$cycles, thus inducing localization in the system. We have supported this result by numerical simulations, detailed in \DIFdelbegin \DIFdel{Section}\DIFdelend \DIFaddbegin \DIFadd{section}\DIFaddend ~\ref{sec:level42},where, $\hat{\sigma}^z$ is detected as an approximate integral invariant ~\cite{Keser2016,Dodonov1978}. \DIFaddbegin \DIFadd{Maintaining the drive parameters at this CDT/DL point, adiabatically changing $\omega$ from small to large values results in a thermal to DMBL phase crossover~\mbox{%DIFAUXCMD
\cite{Mahbub2024}}\hspace{0pt}%DIFAUXCMD
. Therefore, the drive frequency that goes with the CDT/DL point is limited to large values in both $T_2$ and total time period T. To preserve DMBL at the CDT/DL point, we applied a large frequency ($\omega =20$) during numerical investigations, large enough for RWA to hold. Furthermore, to ensure consistency, we set the CDT/DL point to the first root of $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$ in all results.
}\DIFaddend 

\section{\label{sec:level3}Coexistence of DTC \& DMBL}	
The underlying dynamics of the periodic Hamiltonian of the \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend model outlined in \DIFdelbegin \DIFdel{Section}\DIFdelend \DIFaddbegin \DIFadd{section}\DIFaddend ~\ref{sec:mdl_n_dynam} can be understood by employing the Floquet theory (FT). FT allows us to examine the dynamics at the stroboscopic times, namely, at integer multiples of the time period $T$. The effective Floquet Hamiltonian($H^{\mathrm{eff}}$) for the proposed spin-system that undergoes two periods can be written as follows:
\begin{multline}
...
To examine the influence of rotational error on the \DIFdelbegin \DIFdel{chimera order}\DIFdelend \DIFaddbegin \DIFadd{chimeralike state}\DIFaddend , let us begin by considering the scenario where $\epsilon_A=0$. In this situation, the effective Hamiltonian $H^\mathrm{eff}$ in \DIFdelbegin \DIFdel{Eq.}\DIFdelend \DIFaddbegin \DIFadd{equation}\DIFaddend ~\eqref{eq:floq_eff3}  decouples into terms that live separately in regions A and B, \textit{i.e.},
\begin{equation}
...
\begin{figure}[h!]
    \centering
    %DIF > \includegraphics[width=10cm]{figure3_temp.pdf}
    \includegraphics[width=10cm]{figure3.pdf}
    \caption{Floquet quasi-energies of the driven spin-chain,  estimated by numerically diagonalizing $H^{\rm eff}$ in \DIFdelbeginFL \DIFdelFL{Eq.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{equation}\DIFaddendFL ~\eqref{eq:floq_eff3} at 2T for the one dimensional spin-$1/2$ chain. In this simulation, the number of spins $N=8$ and the strong coupling $J_0=0.2/T$ is considered. The quasi-energies are plotted \DIFdelbeginFL \DIFdelFL{for different values of $h,\omega$, the amplitude}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{against $4h/\omega$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{and frequency of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{where }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{periodic }\DIFdelendFL drive \DIFdelbeginFL \DIFdelFL{, respectively.  The x-coordinate plots ${4h}/{\omega}$, where $\omega( = 20)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{frequency $\omega=20$ }\DIFaddendFL is \DIFdelbeginFL \DIFdelFL{kept constant }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fixed, }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{only }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{drive amplitude }\DIFaddendFL $h$ is \DIFdelbeginFL \DIFdelFL{changed, and the y-coordinate plots the corresponding quasi-energies}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{varied}\DIFaddendFL . Level repulsion is minimum when ${4h}/{\omega} = (2n+1)\pi$, $n\in \mathbb{N}_0$. The first \DIFdelbeginFL \DIFdelFL{such point is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{two points are }\DIFaddendFL shown as a vertical black dashed line. Solid red lines indicate the roots of the zeroth-order Bessel function $\mathcal{J}_0(4h/\omega)$.}
    \label{Fig:quasienergy_new}
\end{figure}	

\begin{figure*}[t!]
    \centering
    \DIFaddbeginFL \DIFaddFL{\hspace{2cm}}\DIFaddendFL \includegraphics[width=13.5cm]{figure4.pdf}
    \caption{The time evolution of local magnetization for each spin at region A (\DIFdelbeginFL \DIFdelFL{Site(}\DIFdelendFL i\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL =0,1,2,3) and region B (\DIFdelbeginFL \DIFdelFL{Site(}\DIFdelendFL i\DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL =4,5,6,7) is plotted for $N=8$ spins in a periodically driven one-dimensional spin chain, starting from a fully polarized spin state, with drive frequency $\omega=20$. The spin rotation error $\epsilon_A = 0.03$ and $\epsilon_B = 0.9$, and $g=\pi/T$, where $T=2\pi/\omega$ is the time period of the drive.  In all two panels, different ranges of spin-interactions such as long-range($\beta=0$), intermediate range($\beta=1.5$), short range($\beta=2.5$), and nearest neighbor range($\beta=\infty$), are chosen and plotted, respectively, from left to right, namely (a, b, c, d) in the top row and (e, f, g, h) in the bottom row of each panel. The results of the time evolution for the strong spin coupling ($J_0 = 0.2/T$) \DIFdelbeginFL \DIFdelFL{are shown }\DIFdelendFL in the left panel (P) and the weak spin coupling (\DIFdelbeginFL \DIFdelFL{$J_0 = 0.027/T$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$J_0 = 0.072/T$}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{are shown }\DIFaddendFL in the right panel (Q). The top panels of P \& Q show plots where the drive amplitude $h$ is chosen so that the system lies at the CDT / DL point \DIFaddbeginFL \DIFaddFL{which is chosen to be the first root of $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$}\DIFaddendFL , and the bottom panels show the results away from that CDT/DL point.}
...
\DIFaddbegin \DIFadd{To gain comprehension of the existing phases in the spin chain, we consider the local magnetization at individual sites (i) as the physical order parameter. We utilize it to investigate the regime where TTSB with a sufficiently rigid DTC occurs, detecting the DTC phase in region A and the DMBL phase in region B. Additionally, the spectral decomposition of the local magnetization can be obtained by Fast Fourier Transforms (FFT) of the time-series data. The FFT can be used to identify the DTC phase by detecting peaks corresponding to the subharmonic modes as defined by the DTC criteria (see section~\ref{sec:intro}). }\DIFaddend We now present visuals of the local magnetization, denoted as $\expval{\hat{S}^z_i (t)}$, which is equivalent to the expectation value of the operator $\hat{\sigma}^z_i$ with respect to the state $\ket{\Psi(t)}$, for all sites $i$ in the spin-chain. The data mentioned above have been obtained using numerical simulations of Schr\"odinger dynamics, represented by the equation $\hat{H}(t)\ket{\Psi(t)}= i \hbar \pdv*{\ket{\Psi(t)}}{t}$. This equation describes the behavior of the quantum many-body state $\ket{\Psi(t)}$. Simulations were carried out using QuTiP, a Python-based Quantum Toolbox~\cite{Johansson2013}. The simulations were performed over an extended duration (up to $t = 80T$, with $\hbar$ normalized to unity) and over various ranges of power law interactions ($\beta = 0,1.5,2.5,\infty$). Furthermore, simulations were carried out for weak and strong interaction amplitudes ($J_0 = 0.072/T$ and $J_0 = 0.2/T$ respectively), which are similar to the parameters selected by in previous study~\cite{sakurai_phys_nodate,zhang_observation_2017}. To assess the durability of this \DIFdelbegin \DIFdel{chimera}\DIFdelend \DIFaddbegin \DIFadd{chimeralike state}\DIFaddend , we set the rotational error values as $\epsilon_A = 0.03$ and $\epsilon_B = 0.9$. Furthermore, to observe the emergence of the \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend state in our model, we manipulate the periodic drive in $\hat{H}_2(t)$ by setting a high frequency $\omega=20$ for two different scenarios: one where the parameter $4h/\omega$ is located at \DIFdelbegin \DIFdel{a higher root (third root) of the Bessel function $\mathcal{J}_0(4h/\omega)$, specifically at }\DIFdelend the CDT/DL point, and another where $4h/\omega$  does not lie at any roots of $\mathcal{J}_0$.

As illustrated in \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{figure}\DIFaddend ~\ref{Fig:strong_weak_ea}, the local magnetization exhibits the breaking of discrete time translational symmetry, manifesting a DTC phase in region A at the CDT/DL point, under the condition of strong spin-spin coupling. Simultaneously, region B exhibits localized dynamics. As a result, the coexistence of the DTC and DMBL phases is observed in regions A and B, respectively, leading to the formation of a \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend state. Conversely, it is evident that the system dynamics demonstrate instability upon deviation from the CDT/DL point, irrespective of the particular values of coupling strength($J_0$) that have been chosen. In a similar vein, it can be noted that weak interactions demonstrate the presence of a stable \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend state when they possess a range that encompasses all spins ($\beta=0$). However, when considering intermediate ($\beta = 1.5$), short ($\beta = 2.5$), and nearest-neighbor range interactions ($\beta = \infty$), the DTC melts rapidly inside region A, typically occurring within $20$ cycles of the drive. This result is consistent with the findings of previously published work~\cite{sakurai_phys_nodate}.

The stability of this DTC phase exhibits notable disparities between strong and weak-coupling interactions. At the CDT/DL point, it is seen that the DTC phase exhibits enhanced stability in the presence of strong coupling compared to weak coupling, specifically for interactions that are not long-range in nature. The phenomenon of stable DTC is observed in long-range spin chains, regardless of the strength of spin interactions. Therefore, it is feasible to see the emergence of a \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend state consisting of two separate phases of matter, specifically a time crystal and a localized ferromagnet, simultaneously in a spin-chain system. This phenomenon can occur in typical experimental settings, with long-range interactions being the most suitable candidate.
...
\begin{figure}[h]
	...
	\caption{Fast Fourier Transforms(FFT) of the local magnetization \DIFaddbeginFL \DIFaddFL{$\expval{\hat{S}_z^i}$ }\DIFaddendFL at site $i=1$ in region A of the spin-chain for weak coupling ($J_0 = 0.072/T$, \DIFdelbeginFL \DIFdelFL{left panel}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panel-a}\DIFaddendFL ) and strong coupling ($J_0 = 0.2/T$, \DIFdelbeginFL \DIFdelFL{right panel}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panel-b}\DIFaddendFL ) for different values of $\beta$ as indicated in the legend. The \DIFaddbeginFL \DIFaddFL{frequency $\omega =20$ and }\DIFaddendFL amplitude $h$ is adjusted so that the system is at the \DIFdelbeginFL \DIFdelFL{smallest }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{first root of $\mathcal{J}_0\left(\frac{4h}{\omega}\right)$ }\textit{\DIFaddFL{i.e.}} \DIFaddendFL CDT/DL point. All other parameters are the same as those in \DIFdelbeginFL \DIFdelFL{Fig.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{figure}\DIFaddendFL ~\ref{Fig:strong_weak_ea}. \DIFaddbeginFL \DIFaddFL{In the inset figure of each of the panels, FFT of $\expval{\hat{S}_z^i}$ is plotted in log-scale for smaller $\Omega/\omega$ range. }\DIFaddendFL The peaks at half-integer multiples of \DIFdelbeginFL \DIFdelFL{$\omega$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Omega/\omega$ }\DIFaddendFL denote subharmonic responses in region A that result in \DIFdelbeginFL \DIFdelFL{stable }\DIFdelendFL DTC.}
	\label{Fig:sz_single}
\end{figure}

The suitability of long-range interactions is examined by analyzing the dynamics in frequency space (with the frequency variable denoted by $\Omega$) by applying a Fast Fourier Transform (FFT) on the local magnetism at a specific spin site \DIFdelbegin \DIFdel{$(site(i)=1)$ }\DIFdelend \DIFaddbegin \DIFadd{$(i=1)$ }\DIFaddend in region A. Numerical FFT algorithms provided by the NumPy library\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{harris2020array} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend were utilized to achieve this task\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{harris2020array}}\hspace{0pt}%DIFAUXCMD
...
\begin{figure}[t]
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=15.cm]{figure6.pdf}
%DIFDELCMD < %%%
...
\DIFdelendFL \DIFaddbeginFL \begin{figure}[t]
	\DIFaddendFL \centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 10cm]{figure7.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 15.0cm]{figure8.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Regional }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The temporal variation in regional }\DIFaddendFL magnetization $M^z_{A/B}$ of the \DIFdelbeginFL \DIFdelFL{spin-chain. The magnetization of regions A (red) and B (blue) is plotted as functions of time $t/T$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{spin-1/2 chain }\DIFaddendFL for \DIFdelbeginFL \DIFdelFL{both weak }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{different }\DIFaddendFL spin \DIFdelbeginFL \DIFdelFL{coupling ($J_0=0.072/T$, left panels) and strong spin-coupling }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{rotational errors }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{$J_0=0.2/T$, right panels}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\epsilon_{_{A,B}}$}\DIFaddendFL ) and \DIFdelbeginFL \DIFdelFL{different ranges of }\DIFdelendFL spin \DIFdelbeginFL \DIFdelFL{interactions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{interaction ranges }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{characterized by }\DIFdelendFL $\beta$\DIFdelbeginFL \DIFdelFL{as specified in the legends}\DIFdelendFL ). The \DIFaddbeginFL \DIFaddFL{simulations were conducted for a fixed }\DIFaddendFL time period \DIFdelbeginFL \DIFdelFL{is fixed by setting the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of 80T, with }\DIFaddendFL drive parameters \DIFdelbeginFL \DIFdelFL{$h,\omega$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{set }\DIFaddendFL to \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL CDT/DL point\DIFdelbeginFL \DIFdelFL{, }\textit{\DIFdelFL{i.e.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. The central panel-X illustrates }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{higher root }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{selection }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{$\mathcal{J}_0(4h/\omega)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\epsilon_{A,B}$ values and connects them to the corresponding plots}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{All other parameters are }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The left panels (a-d) show }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{same as }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{plots for different values of $\epsilon_B$ (0.1, 0.3, 0.7, 0.9) with a constant $\epsilon_A = 0.03$. The bottom panels (d-f) show }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{simulations visualized in Fig}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{plots for a constant $\epsilon_B = 0.1$ and varying $\epsilon_A$ (0.03, 0.4, 0.9)}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{~\ref{Fig:strong_weak_ea}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The right panels (g-j) show the plots for a set of $\epsilon_{A,B}$ values that are comparable ($\epsilon_A\sim \epsilon_B$). Weak ($J_0 = 0.072/T = 0.229$) and strong ($J_0 = 0.2/T = 0.637$) spin coupling strengths are considered. The differences in selected $\epsilon_{A,B}$ values are plotted in the $X_y$ and $X_x$ panels}\DIFaddendFL .}
	\DIFdelbeginFL %DIFDELCMD < \label{Fig:regiogionalmag}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{Fig:reg_mag_ea_eb}
\DIFaddendFL \end{figure}
\DIFaddbegin 

\DIFaddend We now examine the regional magnetization of the \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend state, which is given by 
\DIFaddbegin \DIFadd{$M^z_{A/B}\equiv\frac{2}{N}\sum_{i\in{A/B}}\expval{\hat{\sigma}^z_i(t)}$, }\DIFaddend the expectation value  of the total $z-$spin in a specific region. This \DIFdelbegin \DIFdel{can enhance our understanding of }\DIFdelend \DIFaddbegin \DIFadd{will strengthen the case for
}\DIFaddend the suitability of long-range interactions \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{in DTC~\mbox{%DIFAUXCMD
\cite{sakurai_phys_nodate}}\hspace{0pt}%DIFAUXCMD
. We have conducted extensive simulations for various spin rotational errors ($\epsilon_{_{A,B}}$) in order to provide a detailed analysis of the stability of }\DIFaddend the \DIFdelbegin \DIFdel{formation of stable time crystals. The expression for the magnetization of }\DIFdelend \DIFaddbegin \DIFadd{DTC-DMBL chimeralike state. We have explored different values of rotational error by keeping one of the $\epsilon_{_{A,B}}$ constant and adjusting the other from lower to higher values. In addition, we have adjusted both $\epsilon_A$ and $\epsilon_B$ together while keeping them comparable, }\textit{\DIFadd{i.e.}}\DIFadd{, $\epsilon_A \sim \epsilon_B$. The simulations yielded the regional magnetization for times up to $80T$. We have  considered various spin-spin interactions, where $\beta = 0,1.5,2.5,$ and $\infty$. All spins are initially z-polarized. The drive parameters ($h,\omega$) are set to a CDT/DL point with $\omega =20$. 
	}

\DIFadd{The resultant plots can be seen in figure~\ref{Fig:reg_mag_ea_eb}. In this figure, the central primary panel-X shows the selected values of $\epsilon_A$ and $\epsilon_B$. Arrows are drawn in this figure to connect the points in the $\epsilon_A - \epsilon_B$ space to the corresponding $M^z_{A/B}$ plots. Panels 'a' through 'd' show the temporal variation of $M^z_{A/B}$ for different values of $\epsilon_B$ (0.1, 0.3, 0.7, 0.9) with a constant $\epsilon_A = 0.03$. The stability of the DTC-DMBL chimeralike state is observed for all values of $\epsilon_B$ and spin interaction ranges. The DMBL phase remains stable for all spin interaction ranges. However, the DTC phase melts faster as the interaction range shortens. On the other hand, the DTC-melting rate is slower in the case of strong coupling. For smaller values of $\epsilon_B$ (0.7, 0.3, 0.1), the stability of the DMBL phase decreases as $\epsilon_B$ decreases, and the DTC phase melts faster. We have collected data for $M^z_{A/B}$ for comparable values of $\epsilon_{A,B}$ in panels `g', `j', `i', and `h' of figure~\ref{Fig:reg_mag_ea_eb}. As the values of $\epsilon_{A}\sim\epsilon_{B}$ increase, the DTC phase gradually melts and the DMBL phase becomes dominant in both }\DIFaddend regions A and B\DIFdelbegin \DIFdel{is$\displaystyle M^z_{A/B}=\frac{2}{N}\sum_{i\in{A/B}}\expval{\hat{\sigma}^z_i(t)}$~\mbox{%DIFAUXCMD
\cite{sakurai_phys_nodate}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{.
	...
	\DIFaddendFL \centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 10cm]{figure8.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\hspace{1.5cm}}\includegraphics[width = 11cm]{figure9.pdf}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Fast Fourier Transforms(FFT) of regional }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Regional }\DIFaddendFL magnetization \DIFdelbeginFL \DIFdelFL{$M^z_{A}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$M^z_{A/B}$ }\DIFaddendFL of the spin-chain. The magnetization of regions A \DIFdelbeginFL \DIFdelFL{are }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(red) and B (blue) is }\DIFaddendFL plotted as functions of time \DIFdelbeginFL \DIFdelFL{$(\omega/\omega_D)$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t/T$ up to 2000T }\DIFaddendFL for both weak \DIFdelbeginFL \DIFdelFL{spin-coupling }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{spin coupling }\DIFaddendFL ($J_0=0.072/T$, left \DIFdelbeginFL \DIFdelFL{panel}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panels}\DIFaddendFL ) and strong spin-coupling ($J_0=0.2/T$, right panels) and different ranges of spin interactions (characterized by $\beta$ as specified in the legends). The time period is fixed by setting \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL drive parameters $h,\omega$ to the \DIFdelbeginFL \DIFdelFL{first }\DIFdelendFL CDT/DL point, \textit{i.e.} the first root of \DIFdelbeginFL \DIFdelFL{$\mathcal{J}_0(\chi)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathcal{J}_0(4h/\omega)$}\DIFaddendFL . All other parameters are the same as the simulations visualized in \DIFdelbeginFL \DIFdelFL{Fig.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{figure}\DIFaddendFL ~\ref{Fig:strong_weak_ea}.}
	\DIFdelbeginFL %DIFDELCMD < \label{Fig:regionalFFT}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{Fig:regiogionalmag}
\DIFaddendFL \end{figure}
\DIFdelbegin \DIFdel{We expanded }\DIFdelend \DIFaddbegin \DIFadd{In panels `d', `e', `f' of figure~\ref{Fig:reg_mag_ea_eb}, we have plotted $M^z_{A/B}$ for a constant $\epsilon_{B} = 0.1$ and $\epsilon_{A}(=0.03,0.4,0.9)$, respectively. When $\epsilon_{A}=0.03~\&~ \epsilon_{B}=0.1$ (figure~\ref{Fig:reg_mag_ea_eb}-d), we observe that the DTC phase, present in both regions initially, melts simultaneously. As $\epsilon_{A}$ increases, a transition from a melting-DTC to a stable DMBL is observed in region A, as seen in figure~\ref{Fig:reg_mag_ea_eb}-f. Additionally, the stability of the DTC phase in region-B increases with an increase in $\epsilon_{A}$. Thus, a significantly small $\epsilon_{A}$ and large $\epsilon_{B}$ results in the  coexistence of DTC phase in region B and  DMBL phase in region A, manifesting a chimeralike order. For general comprehension, we have calculated the relative spin rotational error $\epsilon^r_{A,B} = \epsilon_{A}-\epsilon_{B}$ and plotted it in the $X_y$ panel, considering constant $\epsilon_{A} = 0.03$, and in the $X_x$ panel, considering constant $\epsilon_{B} = 0.1$. We observe that only at the extreme values of $\epsilon^r_{A,B}$, }\textit{\DIFadd{i.e.}}\DIFadd{, small $\epsilon^r_{A,B} \sim -1.0$ (figure~\ref{Fig:reg_mag_ea_eb}-$X_y$) or  large $\epsilon^r_{A,B} \sim 1.0$, stable chimeralike states have been seen. As $\epsilon^r_{A,B}\rightarrow 0$, the chimeralike state vanishes. For tiny $\epsilon^r_{A,B}$, we observe the DTC phase in region A, and the DMBL in region B. However, for very large values of $\epsilon^r_{A,B}$, we observe the opposite: DMBL in region A and DTC in region B. Therefore, the selection of rotational errors is a crucial element in engineering and stabilizing a DTC-DMBL chimeralike state.
}

\DIFadd{We extended }\DIFaddend the numerical simulations outlined in \DIFdelbegin \DIFdel{Sec.~\ref{sec:level3} }\DIFdelend \DIFaddbegin \DIFadd{earlier sections }\DIFaddend to encompass longer \DIFdelbegin \DIFdel{duration}\DIFdelend \DIFaddbegin \DIFadd{durations, up to $2000T$}\DIFaddend . Consequently, we acquired long-time estimates of the regional magnetization \DIFdelbegin \DIFdel{for regions A and B }\DIFdelend under varying conditions of spin-coupling strength and range. The results are plotted in \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{figure}\DIFaddend ~\ref{Fig:regiogionalmag}. In both cases, the drive parameters were set to the CDT/DL point. In region B, the value of $M^Z_B$ remains constant at unity across all time for all ranges. \DIFdelbegin \DIFdel{In contrast}\DIFdelend \DIFaddbegin \DIFadd{Contrastingly}\DIFaddend , the behavior of $M^Z_A$ in region A is distinct. \DIFdelbegin \DIFdel{In the regime of weak coupling , the persistence of }\DIFdelend \DIFaddbegin \DIFadd{When the coupling is weak, }\DIFaddend $M^Z_A$ \DIFdelbegin \DIFdel{is significant }\DIFdelend \DIFaddbegin \DIFadd{mostly persists }\DIFaddend only for all-to-all interactions\DIFdelbegin \DIFdel{, characterized by $\beta=0$}\DIFdelend . However, for interactions in other ranges, $M^Z_A$ gradually dissipates with time. \DIFdelbegin \DIFdel{In the regime of high coupling , it can be observed that }\DIFdelend \DIFaddbegin \DIFadd{When the coupling is strong, }\DIFaddend the DTC phase \DIFdelbegin \DIFdel{present }\DIFdelend in region A \DIFdelbegin \DIFdel{undergoes a gradual dissolution }\DIFdelend \DIFaddbegin \DIFadd{dissolves gradually }\DIFaddend over time, regardless of the range of spin interactions. 

The \DIFdelbegin \DIFdel{underlying mechanisms by which the DTC disintegrates }\DIFdelend \DIFaddbegin \DIFadd{melting of the DTC phase in $M^Z_A$ manifests as beats, similar to the melting of DTC observed in other systems, such as Stark-MBL~\mbox{%DIFAUXCMD
\cite{Liu2023} }\hspace{0pt}%DIFAUXCMD
and integrable DTCs~\mbox{%DIFAUXCMD
\cite{Chandra2024}}\hspace{0pt}%DIFAUXCMD
.
}\begin{figure}[t]
	\centering
	\DIFaddFL{\hspace{1.5cm}}\includegraphics[width = 13cm]{figure10.pdf}
	\caption{
		\DIFaddFL{Fast Fourier Transforms(FFT) of regional magnetization $M^z_{A}$ of the spin-chain. The magnetization of regions A are plotted as functions of time ($\Omega/\omega$) for both weak spin-coupling ($J_0=0.072/T$, left panel) and strong spin-coupling ($J_0=0.2/T$, right panels) and different ranges of spin interactions (characterized by $\beta$ as specified in the legends). The time period is fixed by setting drive parameters $h,\omega$ to the first CDT/DL point, }\textit{\DIFaddFL{i.e.}} \DIFaddFL{the first root of $\mathcal{J}_0(\chi)$. All other parameters are the same as the simulations visualized in figure~\ref{Fig:strong_weak_ea}. In the inset, the variation in FFT is plotted on a logarithmic scale for a small frequency window.}}
	\label{Fig:regionalFFT}
\end{figure}
\DIFadd{The underlying mechanism }\DIFaddend can be investigated \DIFdelbegin \DIFdel{in frequency space by means of an FFT of regional magnetization . The results, obtained }\DIFdelend \DIFaddbegin \DIFadd{spectrally by performing FFT on the regional magnetization time-series data }\DIFaddend in a manner similar to the FFTs \DIFdelbegin \DIFdel{of the site magnetization in Sec.}\DIFdelend \DIFaddbegin \DIFadd{discussed in section}\DIFaddend ~\ref{sec:level3}, \DIFaddbegin \DIFadd{except with larger datasets, and, consequently, better resolved beats. The results }\DIFaddend are depicted in \DIFdelbegin \DIFdel{Fig.\ref{Fig:regionalFFT}. In the regime of weak spin coupling }\DIFdelend \DIFaddbegin \DIFadd{figure~\ref{Fig:regionalFFT}, where the FFTs were performed on the time-series data shown in figure~\ref{Fig:regiogionalmag}. When the coupling is weak}\DIFaddend , the subharmonic \DIFdelbegin \DIFdel{peaks }\DIFdelend \DIFaddbegin \DIFadd{peak }\DIFaddend at $\Omega=\omega/2$ \DIFdelbegin \DIFdel{exhibit a dominant influence on the continuum of frequency responses in }\DIFdelend \DIFaddbegin \DIFadd{are prominent in frequency space }\DIFaddend $\Omega$\DIFaddbegin \DIFadd{; the secondary peaks are barely noticeable}\DIFaddend . Consequently, \DIFdelbegin \DIFdel{this leads to a }\DIFdelend sustained regional magnetization \DIFaddbegin \DIFadd{is seen }\DIFaddend over an extended period of time \DIFaddbegin \DIFadd{for all-to-all interactions}\DIFaddend . As the parameter $\beta$ is strengthened,  the \DIFdelbegin \DIFdel{continuum of frequencies becomes more }\DIFdelend \DIFaddbegin \DIFadd{subharmonic peak becomes less }\DIFaddend prominent, contributing to \DIFdelbegin \DIFdel{the }\DIFdelend faster disintegration of $M^Z_A$. \DIFdelbegin \DIFdel{In the scenario of strong coupling , the continuum is more }\DIFdelend \DIFaddbegin \DIFadd{When the coupling is strong, the dominant peak is at $\Omega = \omega/2$, however the peaks are much less }\DIFaddend prominent across all ranges, leading to a more rapid disintegration of the regional magnetization compared to the weak spin-coupling. \DIFaddbegin \DIFadd{Therefore, a stable DTC-DMBL chimeralike state persists over a long period, with weak coupling considered in an all-to-all interaction.
}\DIFaddend 
...
\begin{figure}[t!]
    \begin{center}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=10cm]{figure9.pdf}
%DIFDELCMD <     %%%
...
\subsection{\label{sec:level43} \DIFadd{System size dependent stability in chimeralike state}}
\DIFaddend \begin{figure}[t!]
	\begin{center}
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=15cm]{figure10.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=12cm]{figure12.pdf}
	\DIFaddendFL \end{center}
	\caption{\DIFdelbeginFL \DIFdelFL{Weak (left T- panels) and strong (right V-panels) additional static fields defined by $\gamma = J_0/5, J_0$ respectively ( where $J_0=0.2/T$ ), are applied in addition to the strong spin coupling and transverse field at }\DIFdelendFL 
		\DIFaddbeginFL \DIFaddFL{FFT of regional magnetization $M^z_A$ obtained for $8\times10^4$T for }\DIFaddendFL different \DIFaddbeginFL \DIFaddFL{spin }\DIFaddendFL interaction ranges \DIFdelbeginFL \DIFdelFL{set by $\beta = 0.0$ }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{panels a}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\beta = 0,1.5,2.5, \infty[inf]$}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{, $\beta= 1.5$ (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{at }\DIFaddendFL panels \DIFdelbeginFL \DIFdelFL{b)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{from top to bottom}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$\beta=2.5$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{respectively. Weak }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{left }\DIFaddendFL panels\DIFdelbeginFL \DIFdelFL{c}\DIFdelendFL ) \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL and \DIFdelbeginFL \DIFdelFL{$\beta= \infty$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{strong }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{right }\DIFaddendFL panels\DIFdelbeginFL \DIFdelFL{d}\DIFdelendFL ) \DIFaddbeginFL \DIFaddFL{spin couplings are considered}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{frequency of the }\DIFdelendFL drive \DIFdelbeginFL \DIFdelFL{is $\omega = 20$, amplitude $h$ is corresponding to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{parameters are at the }\DIFaddendFL CDT/DL point, \DIFdelbeginFL \DIFdelFL{$\epsilon_A=0.03$, $\epsilon_B=0.9$ and all other parameters are kept similar to previous simulations}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{considering constant $\omega = 20$ with a time period of $T= 2\pi/\omega$}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{local magnetization for each site (i) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{beat frequency $\delta\Omega_B$ }\DIFaddendFL is plotted \DIFdelbeginFL \DIFdelFL{from $t=0$ to $t=80 T$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in the inset figure of each panel for different system sizes N}\DIFaddendFL .
	}
	\DIFdelbeginFL %DIFDELCMD < \label{Fig:robustness}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{Fig:size_dependence}
\DIFaddendFL \end{figure}	
\DIFdelbegin \subsection{%DIFDELCMD < \label{sec:level43} %%%
\DIFdel{Robustness against Static Fields}}
%DIFAUXCMD
...
\DIFdelbegin \DIFdel{The influence }\DIFdelend \DIFaddbegin \DIFadd{We investigated the temporal evolution of $M^z_A$ for different system sizes (N = 4, 6, 8, and 10) over an extended time period of up to $8\times10^4$T. We performed a numerical Fast Fourier Transform (FFT) on the extended time series data. The results are displayed in figure~\ref{Fig:size_dependence} for various spin interaction ranges and coupling strengths. The most prominent peaks indicate frequencies that contribute most to the system dynamics over time. We calculated the beat frequency ($\delta\Omega_B$) 
as the difference between the most prominent peaks on either side of the subharmonic peak at $\Omega/\omega = 1/2$, in a manner similar to~\mbox{%DIFAUXCMD
\cite{Chandra2024}}\hspace{0pt}%DIFAUXCMD
. These were computed for different system sizes (N = 4, 6, 8, 10). Their dependence on system size are shown in the inset of each panel. As the system size increases, the change in $\delta\Omega_B$ diminishes, vanishing in the thermodynamic limit ($\lim_{N\to\infty}\delta\Omega_B= 0$),
...
\subsection{\label{sec:level44} \DIFadd{Robustness against Static Fields}}
\begin{figure}[t!]
	\begin{center}
		\includegraphics[width=13cm]{figure13.pdf}
	\end{center}
	\caption{\DIFaddFL{Weak (left R- panel) and strong (right S-panel) additional static fields defined by $\gamma = J_0/5, J_0$ respectively are applied in addition to the strong ($J_0=0.2/T$) spin coupling and transverse field at different interaction ranges set by $\beta = 0$ (panels a), $\beta= 1.5$ (panels b), $\beta=2.5$ (panels c), and $\beta= \infty$ (panels d). The frequency of the drive is $\omega = 20$, amplitude $h$ is corresponding to CDT/DL point, $\epsilon_A=0.03$, $\epsilon_B=0.9$.  The local magnetization for each site (i) is plotted from $t=0$ to $t=80 T$.}}
	\label{Fig:robustness}
\end{figure}
\DIFadd{The choice }\DIFaddend of the driving parameters $h, \omega$ significantly \DIFdelbegin \DIFdel{affects the long-lasting stability of the DTC-DMBL chimera. The }\DIFdelend \DIFaddbegin \DIFadd{influences the persistence of the }\DIFaddend stability of the \DIFdelbegin \DIFdel{chimera order diminishes even if there is a minor deviation }\DIFdelend \DIFaddbegin \DIFadd{DTC-DMBL chimeralike state, especially when they deviate }\DIFaddend from the CDT/DL point. \DIFdelbegin \DIFdel{This raises questions about the mechanisms by which the chimera resists external fields, even while functioning precisely at }\DIFdelend \DIFaddbegin \DIFadd{When there is a small deviation , the chimera-like order stays stable (see ~\ref{sec:AppendixC} for details). However, the system becomes less stable for larger deviations, as seen in figure~\ref{Fig:strong_weak_ea}. Since the inclusion of weak static fields to a system at CDT/DL point can be qualitatively viewed as deviations of the clean system from }\DIFaddend the CDT/DL point\DIFaddbegin \DIFadd{, we can infer that static fields will notably influence the stability of the DTC-DMBL chimeralike state}\DIFaddend . In order to conduct an investigation \DIFdelbegin \DIFdel{, we will now incorporate }\DIFdelend \DIFaddbegin \DIFadd{of this influence, we incorporated }\DIFaddend the static field \DIFaddbegin \DIFadd{term }\DIFaddend $\hbar\hat{V}(\hat{\sigma}^{\gamma})$ \DIFdelbegin \DIFdel{discussed in Section~\ref{sec:mdl_n_dynam}}\DIFdelend \DIFaddbegin \DIFadd{(as discussed in section~\ref{sec:mdl_n_dynam}) }\DIFaddend into the Hamiltonian $\hat{H}(t)$. This static field remains \DIFdelbegin \DIFdel{un-modulated }\DIFdelend \DIFaddbegin \DIFadd{unmodulated }\DIFaddend by the pulses and acts on the system identically at all times. We \DIFaddbegin \DIFadd{have }\DIFaddend conducted the simulation incorporating this field while all other parameters \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend maintained at the same values at CDT/DL point as in earlier simulations. 

We \DIFdelbegin \DIFdel{ran }\DIFdelend \DIFaddbegin \DIFadd{have run }\DIFaddend simulations for both strong ($\gamma=0.2/T$) and weak ($ \gamma= 0.2/5T$) field amplitudes. The findings are displayed in \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{figure}\DIFaddend ~\ref{Fig:robustness}. At low values of $\gamma$, the influence of spin interactions is more \DIFdelbegin \DIFdel{significant compared to }\DIFdelend \DIFaddbegin \DIFadd{noticeable than the influence of }\DIFaddend the static field, leading to the continued existence of dynamical localization \DIFdelbegin \DIFdel{. Consequently, a stable chimera }\DIFdelend \DIFaddbegin \DIFadd{and a stable chimeralike }\DIFaddend state of the DTC\DIFdelbegin \DIFdel{is observed}\DIFdelend . However, when the value of $\gamma$ increases, the static field \DIFdelbegin \DIFdel{becomes dominant }\DIFdelend \DIFaddbegin \DIFadd{dominates }\DIFaddend over the interactions, resulting in the melting of the DTC after a few cycles. However, it is worth noting that the DMBL phase continues to exist even in the presence of the static field \DIFaddbegin \DIFadd{in a manner similar to DMBL reported in the literature~\mbox{%DIFAUXCMD
\cite{Mahbub2024}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Therefore, a \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend phase known as \textit{Thermal-DMBL} becomes apparent at longer time scales. Therefore, it is possible to achieve a strong DTC-DMBL \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend even when moderate external fields are present.

\section{\label{sec:level7} Summary and Conclusion}
\DIFdelbegin \DIFdel{A new quantum chimera statehas been observed in a closed  system}\DIFdelend \DIFaddbegin \DIFadd{In this work, we have proposed a novel quantum chimeralike state}\DIFaddend , where a discrete-time crystal (DTC) and a dynamically many-body localized (DMBL) phase coexist \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend a one-dimensional \DIFdelbegin \DIFdel{spin-$1/2$ chain. In order to achieve this, we }\DIFdelend \DIFaddbegin \DIFadd{spin-1/2 chain. We }\DIFaddend have divided the lattice into two regions, labeled `A' and `B', based on their rotational errors ($\epsilon_{A/B}$). \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend We have applied two different Hamiltonians, modulated by a time-periodic square wave, which \DIFdelbegin \DIFdel{act }\DIFdelend \DIFaddbegin \DIFadd{acts }\DIFaddend on the spin chain at different times. One Hamiltonian conducts spin-flip operations only in region `A' during \DIFdelbegin \DIFdel{first duty-cycle }\DIFdelend \DIFaddbegin \DIFadd{the first duty cycle }\DIFaddend of the square wave (for time $T_1$), while the other Hamiltonian induces global dynamical localization in the entire spin chain at the second duty cycle (for time $T_2$). This process repeats at every time-period of the square wave, resulting in the coexistence of a DTC phase in region A and a DMBL phase in region B. \DIFdelbegin %DIFDELCMD < } %%%
\DIFdelend We have applied specific high-frequency periodic drive parameters engineered to attain dynamical localization via Coherent Destruction of Tunneling (the CDT/DL point) during the $T_2$ cycle. We have produced the results of numerical simulations that have shown that the interaction on the spin chain plays an essential role in the formulation of the \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend and the stabilization of the DTC phase. Our results suggest that the DTC does not exhibit temporal persistence when the spin-spin coupling strength is weak. This was seen for all types of interaction ranges (except for all-to-all interactions). The presence of a strong spin coupling hinders the relaxation of spins away from the DTC, stabilizing the DTC phase regionally for long times throughout all interaction ranges. Interestingly, the stability of the DTC in region A is contingent on the stability of the DMBL phase in region B. The \DIFaddbegin \DIFadd{weak coupling with }\DIFaddend long-range spin interaction is very robust against the exchange energy per spin, and this enables the \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend to persist for \DIFdelbegin \DIFdel{both weak and strong coupling. The }\DIFdelend \DIFaddbegin \DIFadd{long time. Analytically, the }\DIFaddend inter-region coupling term in the effective Floquet Hamiltonian in \DIFdelbegin \DIFdel{Eq.}\DIFdelend \DIFaddbegin \DIFadd{equation}\DIFaddend ~\ref{eq:floq_eff3} is directly proportional to the spin rotation error $\epsilon_A$. We have tuned $\epsilon_A$ and found that the \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend can persist only if $\epsilon_A$ is small. In the case of larger values, there is an increased level of coupling between the two regions. This heightened coupling results in the immediate melting of the DTC for all interactions, except for long-range interactions.

To further study the resilience of this \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike state }\DIFaddend against static perturbations, an additional external static field is included. Numerical simulations have demonstrated that the system is robust in the face of minor perturbations, as evidenced by the sustained presence of the DTC phase inside region A for an extended duration. \DIFdelbegin \DIFdel{However, when subjected to more significant perturbations , the robustness of the systemdiminishes, leading to rapid dissolution of the DTCphase across all spin-interaction types. The }\DIFdelend \DIFaddbegin \DIFadd{Larger perturbations destabilize the system, dissolving the DTC. Analytically, the }\DIFaddend Floquet Hamiltonian, obtained \DIFdelbegin \DIFdel{analytically and supported by exact simulations}\DIFdelend \DIFaddbegin \DIFadd{in the RWA}\DIFaddend , illustrates the coupling between regions A and B. The entanglement entropy rises at a very slow rate for both weak and strong regional spin interactions. Therefore, the entire system is effectively prevented from thermalizing. Additionally, the persistence of the DTC phase is directly dependent on the stability of the DMBL phase. Our proposed DTC-DMBL \DIFdelbegin \DIFdel{chimera }\DIFdelend \DIFaddbegin \DIFadd{chimeralike }\DIFaddend model shows sustainability even at larger times. \DIFdelbegin \DIFdel{With proper drive parametrization in other 1-dimensional spin models, such as the XY or XXZ models, DTC-DMBL can be realized. The experimental realization of this model and its dynamics can be achieved using }\DIFdelend \DIFaddbegin \DIFadd{In the thermodynamic limit, our proposed model is found to be stable for all types of spin interactions and spin coupling strengths. 
}

\DIFadd{Such chimeralike states can be experimentally realized in systems where DMBL has already been seen. For example, a quantum spin-$1/2$ system consisting of $^{19}\mathrm{F}$ trifluoroiodoethylene in acetone-D6~\mbox{%DIFAUXCMD
\cite{Hegde2014}}\hspace{0pt}%DIFAUXCMD
, as well as }\DIFaddend trapped ions~\cite{sakurai_phys_nodate, Friedenauer2008}, as described in previous studies. \DIFaddbegin \DIFadd{However, there are limits to the resistance to external fields, even while functioning precisely at the CDT/DL point in their absence. 
}\DIFaddend 


\DIFdelbegin %DIFDELCMD < \ack{MR acknowledges The University of Burdwan for support through a state-funded fellowship. AR acknowledges support from the University Grants Commission (UGC) of India, via BSR Startup Grant No. F.30-425/2018(BSR), also from the Science and Engineering Research Board (SERB, Govt. of India) Core Research Grant No. CRG/2018/004002. Additionally, AR thanks Prof. Tanmoy Banerjee, Dept. of Physics, The University of Burdwan, for useful discussions.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Such chimeralike states will be beneficial for quantum engineering and quantum memory design based on DTC~\mbox{%DIFAUXCMD
\cite{zhang_observation_2017}}\hspace{0pt}%DIFAUXCMD
. Furthermore, our model may provide new insights into the design of quantum neural networks, such as quantum reservoir computing systems~\mbox{%DIFAUXCMD
\cite{Fujii_2017, Martinez_2021,Mujal_2021, Akitada2022}}\hspace{0pt}%DIFAUXCMD
.  Additionally, in any chimera state, the part of the system undergoing incoherent dynamics can be viewed as a reservoir that is coupled to the coherently-oscillating system. This coupling must be non-Markovian in nature, since information that is lost to the environment can potentially return to the system~\mbox{%DIFAUXCMD
\cite{Accardi1976,Lindblad1976}}\hspace{0pt}%DIFAUXCMD
. Chimeralike states are therefore useful for studying these types of non-Markovian dynamics in quantum many body physics, extending our understanding of open systems beyond the conventional Markov limit,  where information is permanently lost to the environment~\mbox{%DIFAUXCMD
\cite{Agarwal2012, Gorini1976}}\hspace{0pt}%DIFAUXCMD
. Finally, chimera states have potential applications in modern quantum devices. They can be used in NISQ processors to distinguish between the relatively clean and noisy qubits. Normally, information diffusion starts with the noisy qubits, leading to the quantum state slowly losing coherence into a classical mixed state~\mbox{%DIFAUXCMD
\cite{Bharti2022}}\hspace{0pt}%DIFAUXCMD
. However, using a chimeralike state, the clean qubits can be shielded, effectively extending the coherence time. Thus, our model enables more sophisticated quantum information processing. 
	}
...
\begin{align}
    ...
    \label{eq:hmovap1}
\end{align}
In the penultimate step, we used the Baker-Campbell-Hausdorff formula\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Magnus1954}}\hspace{0pt}%DIFAUXCMD
...
To evaluate $\hat{\mathcal{F}}^2$, we start by calculating each of the four terms in RHS of \DIFdelbegin \DIFdel{Eq.}\DIFdelend \DIFaddbegin \DIFadd{equation~}\DIFaddend \ref{eq:2tprop}. During the $T_1-$cycle, $t\in[0,T/2]$, and the time-independent Hamiltonian $\hat{H}_1$ is applied on spin chain. Thus,
\begin{equation}
    \exp\Big(-\frac{i}{\hbar} \hat{H}_1 T_1\Big) = 	\exp\Bigg[\frac{-i(1-\epsilon_A)\pi}{2}\sum_{l \in A}\hat{\sigma}^x_l\Bigg].
    \label{eq:effham1}
\end{equation}	
It is evident that the effective Hamiltonian for $t \in[T, 3T/2]$ will be same as that obtained from \DIFdelbegin \DIFdel{Eq.}\DIFdelend \DIFaddbegin \DIFadd{equation~}\DIFaddend \eqref{eq:effham1}. Next, during the $T_2-$cycle, $t\in[T/2,T]$, and the time-dependent Hamiltonian, $\hat{H_2}(t)$ is applied. Therefore,
\begin{align}
    ...
    \label{eq:t2hamilt}
\end{align}
Clearly, the effective Hamiltonian for the times  $t \in[3T/2, 2T]$ will also be the same as \DIFdelbegin \DIFdel{Eq.}\DIFdelend \DIFaddbegin \DIFadd{equation~}\DIFaddend \eqref{eq:t2hamilt}. Now,	let us consider the case where $\epsilon_A \neq 0$ and $\epsilon_B=1$. Furthermore, let us define the operators $\displaystyle \hat{V}_{\epsilon_A} \equiv \exp\Big(\frac{i\epsilon_A \pi}{2}\sum_{l\in A}\hat{\sigma}^x_l\Big), \displaystyle \hat{\theta}_l \equiv 2 \left(\sum_{m \in B}J_{lm}\hat{\sigma}^y_m \frac{T}{2} \right)$; the latter describes a rotation acting on operator $\hat{\sigma}^y_l$ in only region B.  Now, we can expand expression for the  $T_2-$cycle propagator in \DIFdelbegin \DIFdel{Eq.}\DIFdelend \DIFaddbegin \DIFadd{equation}\DIFaddend ~\ref{eq:t2hamilt} by splitting the contributions from regions A and B. This yields
\begin{multline}
